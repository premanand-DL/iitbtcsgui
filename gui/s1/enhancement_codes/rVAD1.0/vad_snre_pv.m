function [D, Dsmth, snre_vad, pv_vad, e,segsnr]=vad_snre_pv(dfdata, nfr10, flen, fsh10, ENERGYFLOOR, pv01, pvblk, vadThres)
...
for i=1:nfr10
    ...
    elseif (pvblk(i)==0 || i==nfr10) && sign_pv==1
        ...
        for j=nstart+1:nstop
            postsnr(j) =log10(e(j))-log10(emin);
            if postsnr(j)<0
                postsnr(j)=0;
            end
            D(j)=sqrt(abs(e(j)-e(j-1))*postsnr(j));
        end
        D(nstart)=D(nstart+1);
        ...
        snr_adj=4;
        for j=nstart:nstop
            if Dsmth(j)>Dsmth_thres*vadThres*snr_adj
                snre_vad(j)=1;
            end
        end
    end
...
for i=1:nfr10
    ...
    elseif (snre_vad(i)==0 || i==nfr10) && sign_vad==1
        ...
        pv_vad(nstart:max(j-nexpl-1,1))=0;
        for j=0:(nstop-nstart)
            if pv01(nstop-j)==1
                break;
            end
        end
        pv_vad(nstop-j+1+nexpr:nstop)=0;
    end
end
