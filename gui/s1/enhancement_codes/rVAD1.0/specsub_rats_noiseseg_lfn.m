function [ss,gg,tt,ff,zo]=specsub_rats_noiseseg(si,fsz,noise_seg,pv01,pp)
...
if isstruct(fsz)
    if ne>0
        ...
        [dp,ze]=estnoisem_rats_noiseseg(yp,noise_seg,pv01,ze);       % estimate the noise using minimum statistics
    end
    ssv=fsz.ssv;
else
    if ne>0
        ...
        if size(noise_seg)<nr; noise_seg(nr) = 0; end % to match the size with yp.
        [dp,ze]=estnoisem_rats_noiseseg(yp,noise_seg,pv01,tinc,qp);	% estimate the noise using minimum statistics
    end
...
if ~nr                                  % no data frames
    ...
    %--low frequency noise
    for i=1:nr    
        if sum(yp(i,1:7),2)>sum(yp(i,:),2)/2
            % disp('low freq noise');
            yp(i,1:7)=0;
            out_p(i,1:7)=0;
            out(i,1:7)=0+0i;
