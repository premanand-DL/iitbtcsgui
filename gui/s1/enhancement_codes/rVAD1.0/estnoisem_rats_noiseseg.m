function [x,zo,xs]=estnoisem_rats_noiseseg(yf,noise_seg,pv01,tz,pp)
...
strongN_n = zeros(nr,1);
strongN_nn = zeros(nr,1);
yf_strongN=sum(yf(:,80:129),2);
...
if isempty(yf) && isstruct(tz)             % no real data
    ...
    if isempty(yf)      % provide dummy initialization
        ...
        % loop for each frame
        if sum(pv01(1:10))>=1
            % ypf=sum(yp,2); [Cypf,Iypf] = min(ypf);
            % p=yp(Iypf,:);
        ...
        for t=1:nr              % we use t instead of lambda in the paper
            ...
            if (t>1 && strongN_nn(t)) || noise_seg(t) || (t<11 && sum(pv01(1:10))>=1)
            else
